...
func Test_runScan(t *testing.T) {
	t.Run("happy path", func(t *testing.T) {
		got, err := runScan([]string{"./vul-webhook", "--", "image", "alpine:3.10", "--timeout=30s"}, fakeExecCmdSuccess)
		assert.NoError(t, err)
		assert.Equal(t, "success", string(got))
	})

	t.Run("sad path, no vul args separator found", func(t *testing.T) {
		got, err := runScan([]string{"./vul-webhook", "image", "alpine:3.10"}, fakeExecCmdSuccess)
		assert.Equal(t, "invalid arguments specified", err.Error())
		assert.Equal(t, "", string(got))
	})

	t.Run("sad path, vul fails to run", func(t *testing.T) {
		got, err := runScan([]string{"./vul-webhook", "--", "image", "alpine:3.10"}, fakeExecCmdFailure)
		assert.Equal(t, "exit status 1", err.Error())
		assert.Equal(t, "failure", string(got))
	})
}

func Test_findVulSep(t *testing.T) {
	testCases := []struct {
		name             string
		inputArgs        []string
		...
		{
			name:             "happy path",
			inputArgs:        []string{"./vul-webhook", "--", "image", "alpine:3.10"},
			expectedSepIndex: 2,
		},
		{
		...
		{
			name:             "sad path, separator found but no vul args",
			inputArgs:        []string{"./vul-webhook", "--"},
			expectedSepIndex: -1,
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			assert.Equal(t, tc.expectedSepIndex, findVulSep(tc.inputArgs), tc.name)
		})
	}
}
